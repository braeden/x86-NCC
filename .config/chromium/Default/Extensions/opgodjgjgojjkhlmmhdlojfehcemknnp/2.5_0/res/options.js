...
(function (AlienTube) {
    var Utilities = (function () {
        ...
        */
        Utilities.isYouTubeVideoPage = function () {
    ...
    var Migration = (function () {
        function Migration(lastVersion) {
            ...
            versions.forEach(function (version) {
                _this.migrations[version].call(_this, null);
            });
        }
        return Migration;
    })();
    ...
    var Options = (function () {
        function Options() {
            ...
            this.localisationManager = new AlienTube.LocalisationManager(function () {
                ...
                _this.resetButtonElement = document.getElementById("reset");
                _this.addToExcludeButton = document.getElementById("addSubredditToList");
                /* Set the localised text of the reset button. */
                _this.resetButtonElement.textContent = _this.localisationManager.get("options_label_reset");
                _this.addToExcludeButton.textContent = _this.localisationManager.get("options_button_add");
                /* Get the element for the exclude subreddits input field and the list container. */
                _this.excludeSubredditsField = document.getElementById("addSubredditsForExclusion");
                _this.excludeListContainer = document.getElementById("excludedSubreddits");
                /* Set the page title */
                window.document.title = _this.localisationManager.get("options_label_title");
                AlienTube.Preferences.initialise(function (preferences) {
                    ...
                    for (i = 0, len = Options.preferenceKeyList.length; i < len; i += 1) {
                        ...
                        if (inputElement.tagName === "SELECT") {
                            ...
                            selectValue = AlienTube.Preferences.getString(Options.preferenceKeyList[i]);
                            ...
                            Array.prototype.forEach.call(inputElement.options, function (optionElement) {
                            ...
                            inputElement.addEventListener("change", _this.saveUpdatedSettings, false);
                        }
                            ...
                            inputElement.value = AlienTube.Preferences.getNumber(Options.preferenceKeyList[i]);
                            /* Call the settings changed event when the user has pushed a key, cut to clipboard, or pasted, from clipboard */
                            inputElement.addEventListener("keyup", _this.saveUpdatedSettings, false);
                            inputElement.addEventListener("cut", _this.saveUpdatedSettings, false);
                            inputElement.addEventListener("paste", _this.saveUpdatedSettings, false);
                        }
                        else if (inputElement.getAttribute("type") === "checkbox") {
                            /* This control is a checkbox. Retreive the existing setting for this. */
                            inputElement.checked = AlienTube.Preferences.getBoolean(Options.preferenceKeyList[i]);
                            /* Call the settings changed event when the user has changed the state of the checkbox. */
                            inputElement.addEventListener("change", _this.saveUpdatedSettings, false);
                        }
                    }
                    document.querySelector("label[for='addSubredditForExclusion']").textContent = _this.localisationManager.get("options_label_hide_following");
                    /* Set event handler for the reset button. */
                    _this.resetButtonElement.addEventListener("click", _this.resetSettings, false);
                    ...
                    _this.excludedSubreddits = AlienTube.Preferences.getArray("excludedSubredditsSelectedByUser");
                    /* Erase the current contents of the subreddit list, in case this is an update call on an existing page. */
                    while (_this.excludeListContainer.firstChild !== null) {
                        _this.excludeListContainer.removeChild(_this.excludeListContainer.firstChild);
                    }
                    /* Populate the excluded subreddit list. */
                    _this.excludedSubreddits.forEach(function (subreddit) {
                        _this.addSubredditExclusionItem(subreddit);
                    });
                    ...
                    _this.excludeSubredditsField.addEventListener("keyup", _this.onExcludeFieldKeyUp.bind(_this), false);
                    _this.addToExcludeButton.addEventListener("click", _this.addItemToExcludeList.bind(_this), false);
                    _this.excludeSubredditsField.addEventListener("cut", _this.validateExcludeField.bind(_this), false);
                    _this.excludeSubredditsField.addEventListener("paste", _this.validateExcludeField.bind(_this), false);
        ...
        Options.prototype.addItemToExcludeList = function (event) {
            ...
            setTimeout(function () {
                _this.addToExcludeButton.disabled = true;
                _this.excludeSubredditsField.value = "";
            }, 150);
        };
        /**
        ...
        Options.prototype.removeSubredditFromExcludeList = function (event) {
            ...
            setTimeout(function () {
                _this.excludeListContainer.removeChild(subredditElement);
            }, 500);
        };
        /**
